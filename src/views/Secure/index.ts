...
import WithRender from './secure.html?style=./secure.scss';
...
export default class Secure extends Vue {
  ...
  created () {
    create().subscribe('/getCoreAppsBigScreen', {
      ...
      this.attackMode = result.attackMode.sort(function (a, b) { return parseInt(b.count)-parseInt(a.count);});
      this.attackedApps = result.attackedApps;
      this.attackedDetail = result.attackedDetail;
      this.foreignAttack = result.foreignAttack.sort(function (a, b) { return parseInt(b.count)-parseInt(a.count);});
